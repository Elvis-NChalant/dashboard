...
export const GlareCard = ({
  children,
  className,
  linkHref,  // Add the linkHref prop for the target link
}: {
  children: React.ReactNode;
  className?: string;
  linkHref: string;  // The link to navigate to
}) => {
  ...
  const backgroundStyle = {
    ...
    "--shade":
      "radial-gradient( farthest-corner circle at var(--m-x) var(--m-y),rgba(0,0,0,0.15) 12%,rgba(0,0,0,0.2) 20%,rgba(0,0,0,0.3) 120% ) var(--bg-x) var(--bg-y)/300% no-repeat",
  ...
  return (
    <Link href={linkHref} passHref>
      <div
        ...
        className="relative isolate [contain:layout_style] [perspective:600px] transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-transform w-[200px] h-[200px] cursor-pointer"
        ref={refElement}
        onPointerMove={(event) => {
