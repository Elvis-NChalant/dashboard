...
export default function JobTablePage() {
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      const res = await fetch("http://65.1.112.100:8080/get-table?table=F");
      if (res.ok) {
  ...
  const handleSubmit = async () => {
    ...
    if (editIndex !== null) {
      ...
      const res = await fetch("http://65.1.112.100:8080/add-entry", {
        method: "POST",
        headers: {
    ...
    <>
      <Navbar className="fixed top-0 left-0 w-full z-50 bg-blue-950 shadow-md">
        <NavBody className="flex items-center justify-between">
          <NavbarLogo title="Surface Grinding" />
          <div className="flex-1 flex justify-center">
            <NavItems
              items={navItems.filter((item) => item.name !== "Surface Grinding")}
              className="ml-50"
            />
          </div>
        </NavBody>
