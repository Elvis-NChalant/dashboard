...
export default function JobTablePage() {
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      const response = await fetch("http://52.66.238.110:8080/get-table?table=C");
      if (response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    const requestData = {
      Table: "C",  // Specify the table for Drilling
    ...
    if (editIndex !== null) {
      ...
      const response = await fetch("http://52.66.238.110:8080/add-entry", {
        method: "POST",
        headers: {
    ...
    <>
      ...
      <Navbar className="fixed top-0 left-0 w-full z-50 bg-blue-950 shadow-md">
        <NavBody className="flex items-center justify-between">
          <NavbarLogo title="Jig Boring" />
          <div className="flex-1 flex justify-center">
            <NavItems
              items={navItems.filter((item) => item.name !== "Jig Boring")}
              className="ml-50"
            />
          </div>
        </NavBody>
      ...
      <div className="min-h-screen bg-gray-100 pt-50 pb-24 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {[{ label: "Job ID", name: "jobId", type: "text" },
            ...
            { label: "Jobs Completed", name: "jobsCompleted", type: "number" }].map((field) => (
            <div key={field.name}>
        ...
        <table className="w-full bg-white shadow rounded overflow-hidden mb-6">
          <thead className="bg-gray-200 text-gray-700">
            <tr>
              {["Job ID", "Component", "Assigned Worker", "Operation", "Start Time", "End Time", "Jobs Assigned", "Jobs Completed", "Actions"].map((head) => (
